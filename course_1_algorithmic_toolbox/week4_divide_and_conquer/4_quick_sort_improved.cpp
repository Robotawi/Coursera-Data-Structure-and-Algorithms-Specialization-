...
int main()
{
  ...
  srand(time(0));
  // Randomize the quick sort by randomizing the input sequence permutation

  // Choose randomly an index randy in {0, 1......, n − 1} and swap sequence[0] with sequence[k]\
  then choose randomly an index randy in {1......, n − 1} and swap sequence[1] with sequence[k]\
  then choose randomly an index randy in {2......, n − 1} and swap sequence[2] with sequence[k]; and so on.

  for (int i = 0; i < input_size; i++)
  { 
    int randy = i + (rand() % (input_size - i));
    swap(sequence.at(i), sequence.at(randy));
  }
